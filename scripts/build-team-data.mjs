...
// Gen 6 dex, into public/assets/data/:
//
//   species-gen6.json    [{ num, id, name, types, fullyEvolved }]  national dex 1-721
//   moves-gen6.json      { id: { name, type, category, bp, pp } }
//   learnsets-gen6.json  { speciesId: [moveId, ...] }
//
...
// move itself is a Gen 1-6 move the battle engine supports (present in
 ...
 * Gen 6 types the rest of the app shows. Parse it so the team builder speaks
 * the same language.
 */
...
/**
 * Every Gen 1-6 move (present in `supported`) the species or a pre-evolution
...
async function main() {
  ...
  // moves - German name + Gen 6 type from MOVE_LIBRARY, mechanics from the dex
  const moves = {};
  ...
  // species (base national dex 1-721, no alternate formes) - German names
  const deNames = JSON.parse(await readFile(SPECIES_NAMES_DE, 'utf8'));
  const species = DEX.species
    .all()
    .filter((s) => s.num >= 1 && s.num <= 721 && s.gen <= 6 && !s.forme && !s.isNonstandard)
  ...
  await writeFile(path.join(OUT_DIR, 'moves-gen6.json'), JSON.stringify(moves) + '\n');
  await writeFile(path.join(OUT_DIR, 'species-gen6.json'), JSON.stringify(species) + '\n');
  await writeFile(path.join(OUT_DIR, 'learnsets-gen6.json'), JSON.stringify(learnsets) + '\n');
