...
// src/app/core/models/move.model.ts so it holds *every* Gen 1-5 move, one
// entry per move, sourced from:
//
//   - scripts/move-archetypes.json  -> archetype + Gen 5 type per move
//     (produced by scripts/categorize-moves.mjs)
//   - @pkmn/sim (Dex.forGen(5))     -> canonical id, English name, move number
...
async function main() {
  ...
  // Resolve every mapped move against the Gen 5 dex, sorted by move number
  // so the generated list follows in-game (generation) order.
  const moves = Object.entries(mapping)
    .map(([showdownId, { archetype, type }]) => {
      const m = GEN5.moves.get(showdownId);
      if (!m?.exists) {
        console.warn(`skip: ${showdownId} not found in Gen 5 dex`);
        return null;
      }
  ...
  const block =
    '/**\n' +
    ` * Every Gen 1-5 move, one entry each (${rows.length} total), generated by\n` +
    ' * scripts/generate-move-library.mjs. Archetype + Gen 5 type come from\n' +
